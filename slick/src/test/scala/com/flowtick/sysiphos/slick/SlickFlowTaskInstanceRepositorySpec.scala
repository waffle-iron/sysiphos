...
class SlickFlowTaskInstanceRepositorySpec extends SlickSpec {
  ...
  "Slick Flow Task Instance Repository" should "create instance" in new DefaultRepositoryContext("test-user") {
    ...
    val newInstance: FlowTaskInstance = slickFlowTaskInstanceRepository.createFlowTaskInstance("some-instance", "some-definition", "some-task-id", "log-id", 3, 10, None, Some("onFailure-task"), None)(this).futureValue
    ...
    instancesWithContext.map(_.onFailureTaskId) should contain(newInstance.onFailureTaskId)
    instancesWithContext.head.logId should be("log-id")
  }

  it should "update status" in new DefaultRepositoryContext("test-user") {
    val newInstance: FlowTaskInstance = slickFlowTaskInstanceRepository.createFlowTaskInstance("some-instance_retries", "some-definition", "some-task-id", "log-id", 3, 10, None, None, None)(this).futureValue
