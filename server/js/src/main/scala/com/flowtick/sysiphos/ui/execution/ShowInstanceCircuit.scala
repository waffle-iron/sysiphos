...
import diode.{ Action, Circuit, Effect, EffectSingle }
    ...
    case (model: InstanceModel, action) =>
      action match {
        case LoadInstance(instanceId) =>
          val loadInstance: EffectSingle[FoundInstance] = Effect(api.getInstanceOverview(instanceId).map(overview => FoundInstance(overview)))
