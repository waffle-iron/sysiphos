...
final case class FlowInstanceQuery(
  ...
  status: Option[FlowInstanceStatus] = None,
...
trait FlowInstanceRepository {
  ...
  def createFlowInstance(flowDefinitionId: String, context: Map[String, String], initialStatus: FlowInstanceStatus)(implicit repositoryContext: RepositoryContext): Future[FlowInstanceDetails]
  def counts(flowDefinitionId: Option[Seq[String]], status: Option[Seq[String]], createdGreaterThan: Option[Long]): Future[Seq[InstanceCount]]
}
